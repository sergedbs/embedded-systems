...
#define DHT_RESP_TIMEOUT_US      120
#define DHT_BIT_HIGH_TIMEOUT_US  140
...
esp_err_t dht11_read(float *temperature_c, float *humidity_pct)
{
    ...
    ESP_RETURN_ON_ERROR(wait_for_level(0, DHT_RESP_TIMEOUT_US), "dht11", "resp low timeout");
    ESP_RETURN_ON_ERROR(wait_for_level(1, DHT_RESP_TIMEOUT_US), "dht11", "resp high timeout");
    ESP_RETURN_ON_ERROR(wait_for_level(0, DHT_RESP_TIMEOUT_US), "dht11", "resp end timeout");

    for (int i = 0; i < 40; ++i) {
        ESP_RETURN_ON_ERROR(wait_for_level(1, DHT_RESP_TIMEOUT_US), "dht11", "bit start timeout");

        const int64_t high_start = esp_timer_get_time();
        ESP_RETURN_ON_ERROR(wait_for_level(0, DHT_BIT_HIGH_TIMEOUT_US), "dht11", "bit end timeout");
